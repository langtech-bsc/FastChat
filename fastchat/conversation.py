...
class Conversation:
    ...
    # The system message
    system_message: str = ""
    # The names of two conersation roles
    roles: Tuple[str] = ("USER", "ASSISTANT")
    # BSC: the name of the system role (used with chat_template)
    ...
    def format_instruction(self, instruction: str, instruction_input: str, metadata: dict = None) -> str:
        ...
        prior_context_categories = [ # categories on which context must be shown prior to the instruction
            "closed_qa",
            "qa"
        ]

        random_format = random.choice(formats)
        if "category" in metadata and metadata["category"] in prior_context_categories: # we put the context before the instruction
    ...
    def get_prompt(self, tokenizer=None, metadata: dict = None) -> str:
        ...
        if type(self.system_message) is dict: # BSC: Selecting system prompt based on language of instruction
            lang = None
            if metadata and "lang" in metadata:
                if type(metadata["lang"]) == list: # multilingual instruction
                    for l in ['en', 'es', 'ca']:
                        if l in self.system_message and l in metadata["lang"]:
                            lang = l
                else:
                    if metadata["lang"] in self.system_message:
                        lang = metadata["lang"]
            if not lang: # default language of system prompt
                lang = "en"
            sys_message = self.system_message[lang]
        else:
            sys_message = self.system_message
        system_prompt = self.system_template.format(system_message=sys_message)
        ...
        if self.sep_style == SeparatorStyle.ADD_COLON_SINGLE:
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    ...
                    # BSC modification ------------ Reusing image logic for intruction+input messages
                    if type(message) is list:
                        instruction, input = message
                        if input != "":
                            message = instruction + ' "' + input + '"'
                        else:
                            message = instruction
                    # -----------------------------
                    ...
                    ret += role + ":"
            return ret
        elif self.sep_style == SeparatorStyle.BSC_CHAT_TEMPLATE:
            if system_prompt != "":
    ...
    def copy(self):
        return Conversation(
            ...
            roles=self.roles,
            system_role=self.system_role, #BSC
...
# BSC: conv template for chat_template.
register_conv_template(
    Conversation(
...
# BSC: conv template for chat_template with multilingual system prompt.
register_conv_template(
    Conversation(
        name="bsc_chat_template_system",
        system_message={
            "en" : "A chat between a curious user and an artificial intelligence assistant.\nThe assistant gives helpful, detailed, and polite answers to the user's questions.",
            "es" : "Una conversación entre un usuario curioso y un asistente de inteligència artificial.\nEl asistente da respuestas útiles, detalladas y amables a las preguntas del usuario.",
            "ca" : "Una conversa entre un usuari curiós i un assistent d'intel·ligència artificial. L'assistent dóna respostes útils, detallades i amables a les preguntes del l'usuari."
        },
        roles=("user", "assistant"),
        system_role="system",
        sep_style=SeparatorStyle.BSC_CHAT_TEMPLATE,
